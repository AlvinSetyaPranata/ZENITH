        ...
        <header class="w-full flex items-center justify-between bg-background-700 px-4 py-6">
          ...
          <div class="flex items-center gap-x-4 relative">
            <div class="space-y-1 flex flex-col items-end">
              <h4 class="font-medium">Alvin Setya Pranata</h4>
              <span class="text-sm text-gray-400">Mahasiswa</span>
            </div>

            {/* photo section */}
            <button on:click={() => setProfilePopover((prev) => !prev)} class="flex gap-x-2 items-center hover:cursor-pointer">
            ...
            {/* photo section */}
            {profilePopover() && (
              <button on:click={() => setProfilePopover(false)} class="fixed min-w-screen min-h-screen bg-black/0 z-20 top-0 left-0"></button>
            )}


            {/* profile popover section */}
            <div class={`absolute z-30 right-0 -bottom-36 w-[250px] rounded-md bg-white p-4 text-center space-y-4 ${profilePopover() ? 'opacity-100 transition-opacity duration-300 ease-out' : 'opacity-0 transition-opacity duration-300 ease-in'}`}>
              <A href="/" class="text-black block hover:underline">Akun anda</A>
              <A href="/" class="text-black block hover:underline">Pengaturan</A>
              <A href="/" class="text-red-500 block hover:underline">Keluar</A>
            </div>
            {/* profile popover section */}
          </div>
          {/* profile section */}
